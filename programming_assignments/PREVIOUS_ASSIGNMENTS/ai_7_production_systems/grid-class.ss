...
(define-class (<grid-canvas> parent) (<canvas> parent)                          
    ...
    (public 
      ...
      (make-obstacle-stable-high (x0 y0)
        (let ((x (+ 2 (* x0 size)))
              (y (+ 2 (* y0 size))))
          (set! rect (create <rectangle> self x y (+ x size) (+ y size)))
          (set-fill-color! rect (make <rgb> 0 0 0))))
      (make-obstacle-stable-low (x0 y0)
        (let ((x (+ 2 (* x0 size)))
              (y (+ 2 (* y0 size))))
          (set! rect (create <rectangle> self x y (+ x size) (+ y size)))
          (set-fill-color! rect (make <rgb> 150 150 150))))
      (make-obstacle-unstable-high (x0 y0)
        (let ((x (+ 2 (* x0 size)))
              (y (+ 2 (* y0 size))))
          (set! rect (create <rectangle> self x y (+ x size) (+ y size)))
          (set-fill-color! rect (make <rgb> 150 5 5))))
      (make-obstacle-unstable-low (x0 y0)
        (let ((x (+ 2 (* x0 size)))
              (y (+ 2 (* y0 size))))
          (set! rect (create <rectangle> self x y (+ x size) (+ y size)))
          (set-fill-color! rect (make <rgb> 150 80 80))))))                         
              
(define canvas (create <grid-canvas> top with                                               (background-color: (make <rgb> 215 215 215))))
